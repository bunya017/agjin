<template>
  <q-page padding>
      ...
      <div class="col-12 col-sm-10 col-lg-7 q-mx-xl-xl text-primary" v-if="!notFound">
        ...
        <div class="row q-gutter-y-sm q-pb-md">
          ...
          <div class="col-12 col-sm-6">
            ...
            >
              <q-list>
                <q-item clickable @click="newPerm = true">
                  <q-item-section>
                    <q-item-label>Create and Add Permission to Role</q-item-label>
                  </q-item-section>
                </q-item>
              </q-list>
            </q-btn-dropdown>
    ...
    >
      <q-card class="my-card">
        <q-card-section class="row items-center q-py-sm">
          <div class="text-h5">Create and Add Permission to Role</div>
          <q-space />
          <q-btn v-close-popup outline size="sm" color="primary" label="Close" />
        </q-card-section>
        <q-separator />
        ...
        <q-card-section>
          <form v-on:submit.prevent.stop="createPermissionAndAddToRole" class="q-gutter-md">
            <q-input
              outlined
              ref="name"
              type="text"
              ...
              bottom-slots
              hide-bottom-space
              :error="newPermError.status"
              :error-message="newPermError.message"
              @focus="newPermError.status = false"
  ...
  methods: {
    ...
    createPermissionAndAddToRole () {
      ...
      self.$api.post('/permissions', self.newPermission)
        .then(function (response) {
          const responsePerm = response.data
          self.permsList = self.permsList.concat(responsePerm)
          self.$api.put(
            `/roles/${self.roleObj.name}`,
            { permissions: [responsePerm.name] }
          )
            .then(function (response) {
              self.getRoleDetail()
              self.getAllPermissions()
              ...
              self.addPerm = false
              self.addPermPayload = ''
              self.newPermission = { name: '', description: '' }
        ...
        .catch(function (error) {
          self.newPermError.message = error.response.data.detail
          self.newPermBtnLoading = false
          self.newPermError.status = true
        })
    },
  },
  created () {
